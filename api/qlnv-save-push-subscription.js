...
export default async function handler(req, res) {
  try {
    ...
    if (action === 'save') return await saveSubscription(req, res);
    if (action === 'send') return await sendPush(req, res);
...
async function saveSubscription(req, res) {
  const { manv, diadiem, role, subscription } = req.body || {};
  ...
  return res.status(200).json({ ok: true, action: 'save', data });
}
...
async function sendPush(req, res) {
  ...
  const { title, body, url, role, diadiem, target_manv } = req.body || {};
  ...
  for (const row of data || []) {
    try {
      ...
      results.push({ endpoint: row.endpoint, success: false, error: err.message });
    }
  }
