...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    try {
      ...
      await supabase.from('hoadon_banle').insert([{ sohd, ...hoadon }]);

      const ct_data = chitiet.map(row => ({ sohd, ...row }));
      await supabase.from('ct_hoadon_banle').insert(ct_data);

      await supabase.from('sochungtu').upsert({ loai: prefix, so_hientai: sohd_num });
