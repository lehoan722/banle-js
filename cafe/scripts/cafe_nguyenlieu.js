...
$('#btnSaveNL').onclick=async()=>{
  const id=Number($('#editId').value||0);

  // Nhân viên được thêm mới nhưng không được sửa nguyên liệu đã tồn tại
  if(id && !isAdmin){
    return toast('Chỉ admin được sửa nguyên liệu đã có.');
  }

  const p={
    ma_nguyenlieu:codeFromName($('#fMa').value),
    ten_nguyenlieu:$('#fTen').value.trim(),
    nhom_id:Number($('#fNhom').value),
    don_vi_mua_mac_dinh:$('#fDvt').value.trim(),
    quy_cach_mac_dinh:$('#fQuyCach').value.trim(),
    gia_tham_khao:Number($('#fGia').value||0),
    ghi_chu:$('#fGhiChu').value.trim(),
    updated_by:manv
  };

  if(!p.ma_nguyenlieu || !p.ten_nguyenlieu || !p.don_vi_mua_mac_dinh){
    return toast('Thiếu mã, tên hoặc đơn vị.');
  }

  try{
    if(id){
      await suaNguyenLieu(id,p);
    }else{
    ...
    nguyenlieus=await layNguyenLieu({caAn:true});

    $('#modal').classList.add('hidden');
    renderDanhMuc();

    toast(id ? 'Đã cập nhật nguyên liệu.' : 'Đã thêm nguyên liệu mới.');
  }catch(e){
    console.error(e);
    toast('Lỗi: '+e.message);
  }
...
async function loadHistory() {
    ...
    try {
        const rows = await layLichSu({ tuNgay: $('#tuNgay').value, denNgay: $('#denNgay').value });
