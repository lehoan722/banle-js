...
window.xuatExcelToanBoXNT18 = async function () {
    ...
    async function fetchPage(p) {
        ...
        if (error) {
            ...
            if (msg.includes("PGRST202") || msg.includes("Not Found")) {
                alert("Không tìm thấy RPC 'baocaoxnt18_paged'. Hãy tạo hàm và cấp quyền EXECUTE.");
                throw error;
            }
            throw error;
        }
    ...
    XLSX.utils.book_append_sheet(wb, ws, "XNT18");
    XLSX.writeFile(wb, `baocaoxnt18_${Date.now()}.xlsx`, { compression: false });
};
...
window.moTrangChuyenKho = async () => {
    ...
    const { data: cntData, error: cntErr } = await supabase.rpc('baocaoxnt18_count', countParams);
    ...
    for (let offset = 0; offset < total; offset += pageSize) {
        ...
        const { data, error } = await supabase.rpc('baocaoxnt18_paged', pageParams);
    ...
    window.open('baocaoxnt18_chuyenkho.html', '_blank');
};
