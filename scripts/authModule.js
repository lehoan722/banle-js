...
// NOTE: Đây là anon key (public) dùng cho frontend.
...
export function khoiTaoDangNhapDungChung(options = {}) {
  ...
  function writeEmailHistory(list) {
    try {
      localStorage.setItem('email_suggest_history', JSON.stringify(list.slice(0, 10)));
    } catch {}
  }
  ...
  async function tryEmployeeLogin(cs, manvUpper, password) {
    // Nếu người dùng nhập email (có @) thì bỏ qua bước login nhân viên để tránh gọi /api/login-* (401)
    ...
    // Tương thích code cũ: nhiều trang đọc token/manv/tennv ở localStorage
    localStorage.setItem('diadiem', csFinal);
    localStorage.setItem('supabase_access_token', session.access_token);
    localStorage.setItem('manv', String(nhanvien?.manv || manvUpper).trim().toUpperCase());
    ...
    localStorage.setItem('is_admin', 'false'); // CHỐT: nhân viên luôn false
  ...
  // ===== AUTO: nếu đã có session Supabase -> bỏ qua login overlay + tự vào app =====
  (async () => {
    try {
      ...
      // đảm bảo lưu token tương thích code cũ
      try {
        localStorage.setItem('supabase_access_token', session.access_token || '');
      } catch {}
      ...
      const csAuto =
        ...
        macDinhDiaDiem ||
        'cs1';
      localStorage.setItem('diadiem', csAuto);
      window.diadiem = csAuto;

      const uid = session?.user?.id;

      // phân quyền admin thật sự (RPC)
      const isAdmin = await checkIsAdminBestEffort();
      localStorage.setItem('is_admin', isAdmin ? 'true' : 'false');

      // hydrate profile để tab mới có đủ manv/tennv
      if (isAdmin) {
        // CHỈ admin mới đọc admin_users
        try {
          if (uid) {
            const { data: prof, error: profErr } = await window.supabase
              .from('admin_users')
            ...
            if (!profErr && prof) {
              ...
              // admin nhưng không lấy được profile -> giữ nguyên, KHÔNG ép ADMIN bừa
              localStorage.setItem('is_admin', 'true');
              localStorage.setItem('quyen_sua_hoadon', 'true');
            }
          }
        ...
        // lấy lại đúng manv/tennv theo user_id để tab mới đầy đủ
        try {
          if (uid) {
            ...
            if (!nvErr && nv) {
              ...
              // tiện cho lần sau
              if (!localStorage.getItem('last_login_identifier')) {
                localStorage.setItem('last_login_identifier', String(nv.manv || '').trim());
              }
            }
          }
        } catch {
      ...
      // ✅ QUAN TRỌNG: Tự show app trên tab/cửa sổ mới
      const manvAuto = (localStorage.getItem('manv') || '').trim();
      const tennvAuto = (localStorage.getItem('tennv') || '').trim();
      const isAdminAuto = localStorage.getItem('is_admin') === 'true';
      const suaHDAuto = localStorage.getItem('quyen_sua_hoadon') === 'true';

      const nhanvienLike = {
        manv: manvAuto,
        tennv: tennvAuto,
        is_admin: isAdminAuto,
        sua_hoadon: suaHDAuto
      };

      const context = {
        diadiem: csAuto,
        nhanvien: { manv: manvAuto, tennv: tennvAuto, is_admin: isAdminAuto },
        session
      };

      showAppAfterLogin(nhanvienLike, context);
      return;
    } catch {
      // ignore
    }
...
export async function dangXuatDungChung(options = {}) {
  const { loginContainerId = 'login-container', appContainerId = 'app-container', clearDraft = true } =
    options;
