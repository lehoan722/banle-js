...
async function xuLyNhapNhanhMaSanPham() {
  ...
  const giaTriLoc = (inputFilter?.value || '').toString().trim().toUpperCase();

  if (!giaTriLoc) {
    alert("Bạn cần nhập giá trị lọc trước. Ví dụ: 4-1, GIA27, GIAAO1-1");
    inputFilter?.focus();
    return;
  }
  ...
  try {
    ...
    hot.setDataAtCell(rowIndex, 1, giaTriLoc);
    hot.setDataAtCell(rowIndex, 2, null);

    currentTableMode = 'masp';

    if (previewEl) {
      previewEl.innerHTML = `<span style="color:#16a34a;">✅ Đã thêm mã <b>${masp}</b> với vị trí <b>${giaTriLoc}</b>.</span>`;
    }
...
async function luuDuLieu() {
  ...
  if (!hot || !colSelect) return;

  const colname = colSelect.value;
  if (!colname) {
    alert("Bạn cần chọn mục cần ghi vào trước khi lưu phiên kiểm!");
    return;
  }

  const colLabel = COLS.find(c => c.name === colname)?.label || colname;
  const khuVuc = (inputFilter?.value || '').toString().trim().toUpperCase();

  if (!khuVuc) {
    alert("Bạn cần nhập giá trị lọc/khu vực kiểm trước khi lưu.");
    inputFilter?.focus();
    return;
  }

  let loaiKiem = 'treomau';
  if (colname === 'vitrikho1' || colname === 'vitrikho2') {
    loaiKiem = 'vitrikho';
  }
  ...
  if (rows.length === 0) {
    alert("Không có dữ liệu hợp lệ để lưu phiên kiểm.");
    return;
  }

  if (!confirm(`Lưu phiên kiểm ${rows.length} mã cho mục "${colLabel}" tại khu vực "${khuVuc}"?\n\nDữ liệu sẽ KHÔNG ghi trực tiếp vào dmhanghoa.`)) {
    return;
  }
  ...
  try {
    ...
    const { data: phien, error: phienError } = await supabase
      ...
      .insert([{
        ...
        ghichu: `Tạo từ trang sửa danh mục hàng hóa nhân viên - ${colLabel}`
      }])
      .select("id, ma_phien")
      .single();
    ...
    if (previewEl) {
      previewEl.innerHTML =
        `<span style="color:#16a34a;">✅ Đã lưu phiên kiểm <b>${maPhien}</b>, tổng <b>${rows.length}</b> dòng. Không cập nhật trực tiếp danh mục hàng hóa.</span>`;
    }
