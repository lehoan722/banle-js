...
window.taiBaoCaoXNT = async function () {
    ...
    const functionName = document.getElementById("selectFunction")?.value || "baocaoxnt14ccn_paged";
    ...
    const cbTonghop = document.getElementById('tonghopSizeCheckbox');
    const cbChiTiet = document.getElementById('chitietSizeCheckbox');
    if (cbTonghop && cbChiTiet) {
        cbTonghop.addEventListener('change', () => { if (cbTonghop.checked) cbChiTiet.checked = false; });
        cbChiTiet.addEventListener('change', () => { if (cbChiTiet.checked) cbTonghop.checked = false; });
    }

    const chitietSize = document.getElementById("chitietSizeCheckbox").checked;
    ...
    const params = {
        ...
        p_dsmsp: finalMaspList,
        p_tonghop_size: chitietSize ? false : tonghopSize,   // tránh bật cả 2
        p_detail_size: chitietSize,
        p_limit: pageSize,
        p_offset: (currentPage - 1) * pageSize
  ...
  `;
    ...
    const columns = [
        ...
        { data: 'khocs1', title: 'Vị trí CS1', width: 80 },
        { data: 'khocs2', title: 'Vị trí CS2', width: 80 },
    ...
    hotInstance = new Handsontable(container, {
        ...
        cells(row, col) {
            ...
            const sd = inst.getSourceDataAtRow(row);
            const props = {};
            if (sd && (sd.size === 0 || sd.size === '0')) {
                props.className = (props.className ? props.className + ' ' : '') + 'size-zero';
            }
            return props;
        },
...
window.xuatExcelToanBo = async function () {
    ...
    while (hasMore) {
        const { data, error } = await supabase.rpc("baocaoxnt14ccn_paged", {
            ...window.lastParams,
            p_limit: pageSize,
