...
const hotCols = [
  ...
  { data: "so_ngay_khong_ban", title: "KHÔNG BÁN (NGÀY)", type: "numeric", width: 110 },
...
function renderHOT() {
  ...
  hot = new Handsontable(container, {
    ...
    cells: (row, col) => {
      ...
      if (
        ["tong_nhap", "tong_xuat", "ton_hientai", "tyle_ton", "so_ngay_khong_ban", "stt"].includes(
          key
        )
      ) {
...
function updateDetailPanel(rec) {
  ...
  $("#infoNhom").textContent = `${rec.nhomhang || ""} / ${rec.chungloai || ""}`.trim() || "-";
  $("#infoMauNhacc").textContent = `${rec.mausac || ""} / ${rec.nhacc || ""}`.trim() || "-";
  ...
  $("#infoTyle").textContent = rec.tyle_ton != null ? formatNumber(rec.tyle_ton * 100, 1) + " %" : "-";
  $("#infoLastSale").textContent = formatDate(rec.ngay_ban_cuoi);
  $("#infoNoSale").textContent = rec.so_ngay_khong_ban != null ? `${rec.so_ngay_khong_ban} ngày` : "-";
  ...
  // Bạn sửa lại base URL theo bucket ảnh thực tế
...
async function runXaHang() {
  ...
  try {
    ...
    const { data, error } = await supabase.rpc("goiy_xahang", {
      p_den_ngay: denNgay,
    ...
    if (hotData.length === 0) {
      ...
      $("#infoTable").querySelectorAll("td").forEach((td) => (td.textContent = "-"));
      $("#xaImgBox").innerHTML =
...
function main() {
  ...
  ["denNgay", "ngayKhongBan", "tonMax", "tyleMax", "nhomhangFilter", "chungloaiFilter", "nhaccFilter"].forEach(
    (id) => {
      const el = $("#" + id);
      if (!el) return;
