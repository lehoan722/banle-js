...
export async function khoiTaoUngDung() {
    ...
    // --- 1. CẤU HÌNH TỌA ĐỘ CỬA HÀNG & DANH SÁCH TRANG BẮT BUỘC ĐỊNH VỊ ---
    const STORE_LOCATIONS = {
        cs1: [
            { lat: 21.5525047, lng: 105.8423559 }, // CS1 – Tích Lương
        ],
        cs2: [
            { lat: 21.5843348, lng: 105.8343116 }, // CS2 – Lương Ngọc Quyến
        ],
    };

    /**
     * Mỗi phần tử là 1 TRANG cần guard vị trí.
     * pattern: chuỗi để match trong URL (path.includes(pattern))
     * storeKey: 'cs1' | 'cs2'
     * requireMobile: true = chỉ cho phép điện thoại/tablet, false = mọi thiết bị
     */
    const LOCATION_GUARDS = [
        // === BÁN NHÂN VIÊN: CHỈ ĐIỆN THOẠI + ĐÚNG VỊ TRÍ ===
        { pattern: "bannvcs1", storeKey: "cs1", requireMobile: true },
    ...
    // Tìm config tương ứng với trang hiện tại
    const guardConfig = LOCATION_GUARDS.find(cfg => path.includes(cfg.pattern));

    if (guardConfig) {
        // 1A. Nếu trang yêu cầu thiết bị di động thì kiểm tra
        if (guardConfig.requireMobile && !isMobileDevice()) {
            alert("Trang này chỉ được dùng trên điện thoại tại cửa hàng.");
        ...
        // 1B. Kiểm tra vị trí trong cửa hàng
        const storePoints = STORE_LOCATIONS[guardConfig.storeKey] || [];
        let inStore = await checkInStoreLocation(storePoints);

        if (!inStore) {
            // checkInStoreLocation() đã alert lý do, ở đây chỉ ẩn app và dừng
            try {
                const app = document.getElementById("app-container");
                const login = document.getElementById("login-container");
                if (app) app.style.display = "none";
        ...
        // 1C. TỰ ĐỘNG KIỂM TRA LẠI VỊ TRÍ ĐỊNH KỲ (5 PHÚT/LẦN)
        setInterval(async () => {
            const stillInStore = await checkInStoreLocation(storePoints);
