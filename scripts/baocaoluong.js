...
async function tinhLuongThang() {
  ...
  try {
    // 1) Lấy giờ công tháng từ chamcong_tinhcong_monthly (gộp tất cả cơ sở)
    const { data: chamcongData, error: chamcongError } = await supabase.rpc(
      "chamcong_tinhcong_monthly",
      {
        tu_ngay: tuNgay,
        den_ngay: denNgay,
        p_diadiem: null, // tất cả chi nhánh
        p_manv: manv
      }
    );
    ...
    if (kpiData && kpiData.length > 0) {
      // Hàm này trả 1 dòng với tong_doanh_thu, tong_sl_web, ty_le_ghep, so_sp_khac_gia, tb_phut
...
document.addEventListener("DOMContentLoaded", () => {
  setDefaultDates();
  setStatus("Chọn khoảng ngày, nhập mã nhân viên, lương/giờ và % thưởng rồi bấm Tính lương.");
