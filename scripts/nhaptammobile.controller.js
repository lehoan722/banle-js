...
;(() => {
  ...
  async function onEnterMa(e){
    ...
    e.preventDefault()
    $('#inpMa').value = $('#inpMa').value.trim().toUpperCase()
    const masp = $('#inpMa').value
    if(!masp) return
    // kiểm mã có trong cache
    if(!cacheSKU.has(masp)){
      window.AppAudio?.warn?.()
      $('#unknownSku').style.display = 'block'
      $('#unknownSku').textContent = `Mã không có trong danh mục: ${masp}`
      return
    } else {
      $('#unknownSku').style.display = 'none'
    }
    await applyQuanLySizeForCurrentMa()
    // nhảy đến size đầu được phép
    if($('#q0') && !$('#q0').disabled){ $('#q0').focus(); $('#q0').select(); return }
    $('#q38').focus(); $('#q38').select()
  }
  ...
  function handleChuyen2(){
    ...
    for(const ln of lines){
      ...
      const masp = tokens[0].toUpperCase()
      if(masp.length<4){ unknown.add(masp); continue }
      if(!cacheSKU.has(masp)){ unknown.add(masp); continue }
      if(!push.has(masp)) push.set(masp, {})
      for(let i=1;i<tokens.length;i++){
        const t = tokens[i]; const n = parseInt(t,10)
    ...
    // đổ xuống
    const listMasp = []
    for(const [m,qtyMap] of push.entries()){
      listMasp.push(m)
      NTGrid.setRow(m, { qty: qtyMap })
    }
    if(listMasp.length) fetchVitriTonBatch(listMasp)
    // cảnh báo mã lạ
    const warn = [...unknown].filter(x=>x && x.length>=4)
    const box = $('#unknownSku')
    if(warn.length){ box.style.display='block'; box.textContent = 'Mã không có DM: ' + warn.join(', ') }
