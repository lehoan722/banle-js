...
async function luuDuLieu() {
  ...
  if (!colname) {
    alert("Bạn cần chọn mục cần ghi vào trước khi lưu.");
    return;
  }

  let mode = "";

  if (colname === "treomaucs1" || colname === "treomaucs2") {
    mode = "treomau";
  } else if (colname === "vitrikho1" || colname === "vitrikho2") {
    mode = "vitrikho";
  } else {
    alert("Mục này chưa được cấu hình để lưu sang Google Sheet.");
    return;
  }
  ...
  const rows = source
    .map(r => ({
      masp: (r.masp || "").toString().trim().toUpperCase(),
      value: (r[colname] || "").toString().trim()
    }))
    .filter(r => r.masp && r.value);

  if (rows.length === 0) {
    alert("Không có dữ liệu hợp lệ để lưu. Cần có mã sản phẩm ở cột A và giá trị ở cột B.");
    return;
  }

  const tenBang = mode === "treomau" ? "BAYMAUCS1" : "KIEMKHOCS1";

  if (!confirm(`Bạn có chắc muốn lưu ${rows.length} dòng vào Google Sheet "${tenBang}" không?`)) {
    return;
  }

  try {
    if (previewEl) {
      previewEl.innerHTML = `<span>⏳ Đang lưu <b>${rows.length}</b> dòng vào Google Sheet <b>${tenBang}</b>...</span>`;
    }
    ...
    if (!result.ok) {
      ...
      if (previewEl) {
        previewEl.innerHTML = `<span style="color:#dc2626;">❌ Lưu thất bại: ${result.message}</span>`;
      }
      return;
    }
    ...
    hot.batch(() => {
      for (let r = 0; r < hot.countRows(); r++) {
        ...
        const val = (hot.getDataAtCell(r, 1) || "").toString().trim();
        if (masp && val) {
          hot.setDataAtCell(r, 2, "OK");
        }
      }
    });

    if (previewEl) {
      previewEl.innerHTML = `<span style="color:#16a34a;">✅ ${result.message}</span>`;
    }
    ...
    console.error(err);
    alert("Lỗi khi lưu Google Sheet: " + (err?.message || err));
    if (previewEl) {
      previewEl.innerHTML = `<span style="color:#dc2626;">❌ Lỗi khi lưu Google Sheet.</span>`;
    }
  }
}
