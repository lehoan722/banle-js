...
window.taiBaoCaoXNT = async function () {
    ...
    let cnt = 0;
    try {
        const res = await supabase.rpc("baocaoxnt16_count", base);
...
async function loadXNTPage(page) {
    const functionName = document.getElementById("selectFunction")?.value || "baocaoxnt16_paged";
    ...
    let tong = { dauky: 0, nhapmua: 0, tongnhap: 0, xuatban_cs1: 0, xuatban_cs2: 0, xuatban: 0, tongxuat: 0, cuoiky: 0 };
    data.forEach(r => {
        tong.dauky += r.dauky || 0;
        tong.nhapmua += r.nhapmua || 0;
        tong.tongnhap += r.tongnhap || 0;
        tong.xuatban_cs1 += r.xuatban_cs1 || 0;
        tong.xuatban_cs2 += r.xuatban_cs2 || 0;
    ...
    document.getElementById('xntSummary').innerHTML =
      ...
      Xuất bán CS1: <b>${tong.xuatban_cs1.toLocaleString()}</b> | Xuất bán CS2: <b>${tong.xuatban_cs2.toLocaleString()}</b> | Xuất bán (gộp): <b>${tong.xuatban.toLocaleString()}</b> |
    ...
    const columns = [
        ...
        { data: 'xuatban_cs1', title: 'Xuất bán CS1', width: 82, className: 'htRight', renderer: zeroBlankRenderer },
        { data: 'xuatban_cs2', title: 'Xuất bán CS2', width: 82, className: 'htRight', renderer: zeroBlankRenderer },
        { data: 'xuatban', title: 'Xuất bán (gộp)', width: 90, className: 'htRight', renderer: zeroBlankRenderer },
...
window.xuatExcelToanBoXNT15 = async function () {
  ...
  const headers = [
    "STT","Mã hàng","Kích cỡ","Xuất bán CS1","Xuất bán CS2","Xuất bán (gộp)","Tồn CS1","Tồn CS2",
  ...
  for (let p = 0; p < all.length; p++) {
    ...
    for (let i = 0; i < pageRows.length; i++) {
      ...
      aoa.push([
        sttOffset + i + 1,
        r.masp ?? "", r.size ?? "",
        r.xuatban_cs1 ?? 0, r.xuatban_cs2 ?? 0, r.xuatban ?? 0, r.ton_cs1 ?? 0, r.ton_cs2 ?? 0,
