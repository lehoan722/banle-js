...
const COLS = [
  { name: "vitri_thucte", label: "Vị trí thực tế" }
];

function getKiemInfo() {
  const coso = (document.getElementById("kiem-coso")?.value || currentLoginCoSo || "cs1").trim();
  const loaiKiem = (document.getElementById("kiem-loai")?.value || "treomau").trim();
  const khuVuc = (document.getElementById("kiem-khuvuc")?.value || "").trim().toUpperCase();

  return {
    coso,
    loaiKiem,
    khuVuc,
    colname: "vitri_thucte"
  };
}

function taoMaPhienKiem() {
  const d = new Date();
  ...
  const manv = (currentLoginManv || localStorage.getItem("manv") || sessionStorage.getItem("manv") || "NV").toUpperCase();

  return `KVT_${yyyy}${mm}${dd}_${hh}${mi}${ss}_${manv}`;
}
...
async function xuLyNhapNhanhMaSanPham() {
  ...
  const { khuVuc, colname } = getKiemInfo();

  if (!hot) return;

  if (!khuVuc) {
    alert("Bạn cần nhập khu vực đang kiểm trước. Ví dụ: GIAAO1-1 hoặc KHO-K1");
    document.getElementById("kiem-khuvuc")?.focus();
    return;
  }
  ...
  try {
    ...
    hot.setDataAtCell(rowIndex, 1, khuVuc);
...
function attachUIEvents() {
  const btnPhanTich = document.getElementById('btn-phan-tich');
  const colSelect = document.getElementById('col-select');
  ...
  if (btnPhanTich) {
    btnPhanTich.onclick = hienThiPhanTich;
  }
...
async function luuDuLieu() {
  const previewEl = document.getElementById("preview");
  const { coso, loaiKiem, khuVuc, colname } = getKiemInfo();

  if (!hot) return;

  if (!khuVuc) {
    alert("Bạn cần nhập khu vực đang kiểm trước khi lưu.");
    document.getElementById("kiem-khuvuc")?.focus();
    return;
  }
  ...
  const rows = hot.getSourceData()
    .map(r => ({
      masp: (r.masp || "").toString().trim().toUpperCase(),
      vitri_thucte: (r[colname] || "").toString().trim().toUpperCase()
    }))
    .filter(r => r.masp && r.vitri_thucte);
  ...
  if (!confirm(`Lưu phiên kiểm ${rows.length} mã tại khu vực ${khuVuc}?`)) {
    return;
  }

  try {
    const maPhien = taoMaPhienKiem();

    if (previewEl) {
      previewEl.innerHTML = `⏳ Đang lưu phiên kiểm <b>${maPhien}</b>...`;
    }

    const { data: phien, error: phienError } = await supabase
      ...
      .insert([{
        ...
        loai_kiem: loaiKiem,
        khu_vuc: khuVuc,
        manv: currentLoginManv || localStorage.getItem("manv") || sessionStorage.getItem("manv") || null,
        tennv: currentLoginTenNv || localStorage.getItem("tennv") || sessionStorage.getItem("tennv") || null,
        ghichu: null
      }])
      .select("id, ma_phien")
      .single();

    if (phienError) throw phienError;
    ...
    hot.batch(() => {
      for (let r = 0; r < hot.countRows(); r++) {
        const masp = (hot.getDataAtCell(r, 0) || "").toString().trim();
        const vitri = (hot.getDataAtCell(r, 1) || "").toString().trim();
        if (masp && vitri) {
          hot.setDataAtCell(r, 2, "ĐÃ LƯU");
        }
      }
    });
    ...
    if (previewEl) {
      previewEl.innerHTML =
        `<span style="color:#16a34a;">✅ Đã lưu phiên kiểm <b>${maPhien}</b>, tổng <b>${rows.length}</b> dòng.</span>`;
    }

    await phanTichSauKhiLuu();

  } catch (err) {
    console.error(err);
    alert("Lỗi khi lưu phiên kiểm: " + (err?.message || err));
    if (previewEl) {
      previewEl.innerHTML = `<span style="color:#dc2626;">❌ Lỗi khi lưu phiên kiểm.</span>`;
    }
  }
}

async function phanTichSauKhiLuu() {
  const previewEl = document.getElementById("preview");
  const { coso, loaiKiem } = getKiemInfo();

  try {
    const { data: nhieuVitri, error: err1 } = await supabase
      .from("v_kiem_vitri_nhieu_vitri")
      .select("*")
      .eq("coso", coso)
      .eq("loai_kiem", loaiKiem)
      .limit(20);

    if (err1) throw err1;

    const { data: saiChuan, error: err2 } = await supabase
    ...
    const soNhieu = nhieuVitri?.length || 0;
    const soSai = saiChuan?.length || 0;

    if (previewEl) {
      previewEl.innerHTML +=
        `<br><span>📊 Phân tích nhanh: ` +
        `có <b style="color:#dc2626;">${soNhieu}</b> mã nhiều vị trí, ` +
        `<b style="color:#ea580c;">${soSai}</b> dòng sai/chưa có vị trí chuẩn.</span>`;
    }

    if (soNhieu > 0 || soSai > 0) {
      console.log("Mã nhiều vị trí:", nhieuVitri);
      console.log("Sai/chưa có vị trí chuẩn:", saiChuan);
    }

  } catch (err) {
    console.warn("Lỗi phân tích sau kiểm:", err);
  }
}

async function hienThiPhanTich() {
  await phanTichSauKhiLuu();
  alert("Đã phân tích xong. Chi tiết tạm xem trong Console, bước sau sẽ làm bảng báo cáo riêng.");
}
