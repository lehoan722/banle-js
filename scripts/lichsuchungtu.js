...
async function enrichEditRows(rows) {
    const arr = Array.isArray(rows) ? rows : [];
    if (!arr.length) return arr;

    const sohdList = [...new Set(arr.map(r => String(r.sohd || '').trim()).filter(Boolean))];
    if (!sohdList.length) return arr;

    const headerMap = new Map();

    // 1) Ưu tiên lấy từ hoadon_banle hiện tại
    const { data: hdData, error: hdErr } = await supabase
        .from('hoadon_banle')
        .select('sohd, loaihd, diadiem, manv, tennv')
        .in('sohd', sohdList);

    if (!hdErr && Array.isArray(hdData)) {
        hdData.forEach(r => {
            headerMap.set(String(r.sohd), {
                loaihd: r.loaihd || '',
                diadiem: r.diadiem || '',
                nguoi_ct: r.tennv || r.manv || ''
            });
        });
    }

    // 2) Nếu chưa có thì fallback từ invoice_version_history.header
    const missingSohd = sohdList.filter(s => !headerMap.has(s));
    if (missingSohd.length) {
        const { data: verData, error: verErr } = await supabase
            .from('invoice_version_history')
            .select('sohd, saved_at, header')
            .in('sohd', missingSohd)
            .order('saved_at', { ascending: false });

        if (!verErr && Array.isArray(verData)) {
            for (const r of verData) {
                const sohd = String(r.sohd || '');
                if (headerMap.has(sohd)) continue;
                const h = r.header || {};
                headerMap.set(sohd, {
                    loaihd: h.loaihd || '',
                    diadiem: h.diadiem || '',
                    nguoi_ct: h.tennv || h.manv || ''
                });
            }
        }
    }
    ...
    return arr.map(r => {
        const extra = headerMap.get(String(r.sohd || '')) || {};
        return {
            ...r,
            loaihd_view: extra.loaihd || '',
            diadiem_view: extra.diadiem || '',
            nguoi_ct_view: extra.nguoi_ct || '',
            nguoi_thuc_hien_view: r.actor_uid || ''
        };
    });
}
...
async function loadEditLogs() {
    ...
    rows = await enrichEditRows(rows);
...
function renderEditLogs(rows) {
  ...
  `;
    ...
    if (!rows.length) {
        $('mainTbody').innerHTML = `<tr><td colspan="12" class="empty">Không có dữ liệu.</td></tr>`;
        return;
    }
...
function openLogModal(row) {
    ...
    <div><b>Thời gian:</b> ${formatDateTime(row.logged_at)}</div>
