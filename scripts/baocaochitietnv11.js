...
window.taiBaoCaoChiTiet = async function () {
    ...
    const { data: cnt, error: errCnt } = await supabase.rpc("baocaochitietnv11_bh_count_v2", currentFilters);
    if (errCnt) {
...
async function taiTrang(page) {
    ...
    const { data, error } = await supabase.rpc("baocaochitietnv11_bh_page_v2", params);
