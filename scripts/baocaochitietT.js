...
window.taiBaoCaoChiTiet = async function () {
    ...
    const { data: meta, error: errCnt } =
        await supabase.rpc("baocaochitiet_bht_count", currentFilters);
    if (errCnt) { console.error(errCnt); alert("Lỗi đếm dữ liệu!"); return; }

    const m = meta && meta[0] ? meta[0] : { total_rows: 0, tongsl: 0, tongtien: 0 };
    totalRows = Number(m.total_rows || 0);

    // hiện “dòng tổng” ngay tại đây
    document.getElementById("tonghop").innerText =
        `Tổng SL: ${Number(m.tongsl || 0).toLocaleString('vi-VN')}  |  ` +
        `Tổng tiền: ${Number(m.tongtien || 0).toLocaleString('vi-VN')}`;
